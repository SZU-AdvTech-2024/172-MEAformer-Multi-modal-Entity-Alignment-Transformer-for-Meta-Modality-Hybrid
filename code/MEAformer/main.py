...
from rule.adjacency_matrix import SpRelWeiADJ
from rule.rule_loss import RuleDataset
from rule.cross_graph_completion import CrossGraphCompletion, print_time_info
...
import gc
import copy
from rule.triple_graph import TripleDataset

class Runner:
    def __init__(self, args, writer=None, logger=None, rank=0):
        self.rules_data_tg = None
        self.rules_data_sr = None
        #TODO:=========================
        self.sp_twin_adj=None
        self.cgc = None
        self.non_acylic=False
        ...
        ## rank is the gpu and dist is distribute
        self.rank = rank
        self.nega_n_r = 2
        self.args = args
        self.writer = writer
        ...
        self.data_init()
        if args.rule:
            logger.info("train with rule loss!")
            self.rule_init(r"D:\AAA\code\MEAformer\data\mmkg\DBP15K\fr_en", False)
        if args.rel_weight:
            logger.info("train with rel_weight!")
            self.rule_init(r"D:\AAA\code\MEAformer\data\mmkg\DBP15K\fr_en", True)
        if args.rel_weight:
            self.rel_weight()
        self.model_choise()
        set_seed(args.random_seed)
    ...
    def model_choise(self):
        ...
        if self.args.model_name == "MEAformer":
            self.model = MEAformer(self.KGs, self.args, self.sp_twin_adj)

        print(self.model)
        # self.args.model_name_save = "MEAformer_FBDB15K_0.8_v1_norm_0.8_il500_b500__non_iter"
    ...
    def rule_init(self, directory, load=False):
        ...
        if load:
            try:
                ...
                self.cgc = CrossGraphCompletion(directory, self.train_ill, self.triples,  self.rule_transfer, # TODO : be a parameters
                                                self.graph_completion)
                self.cgc.init()
                self.cgc.save(directory / 'running_temp')
        else:
            print_time_info('CrossGraphCompletion start...')
            self.cgc = CrossGraphCompletion(directory, self.train_ill, self.triples, self.rule_transfer,
                                            # TODO : be a parameters
                                            self.graph_completion)
    ...
    def data_init(self):
        self.KGs, self.non_train, self.train_set, self.eval_set, self.test_set, self.test_ill_ , self.ent2id_dict, self.triples = load_data(self.logger, self.args)
        self.train_ill = self.train_set.data


        self.eval_left = torch.LongTensor(self.eval_set[:, 0].squeeze()).to(self.args.device)
        self.eval_right = torch.LongTensor(self.eval_set[:, 1].squeeze()).to(self.args.device)
        if self.test_set is not None:
            self.test_left = torch.LongTensor(self.test_ill[:, 0].squeeze()).to(self.args.device)
            self.test_right = torch.LongTensor(self.test_ill[:, 1].squeeze()).to(self.args.device)
    ...
    def rel_weight(self):
        # TODO: ============================================================================================
        self.sp_twin_adj = SpRelWeiADJ(self.cgc, self.non_acylic, cuda=True if self.args.device == "cuda" else False)
    ...
    def run(self):
        ...
        self.stage = 0
        if self.args.rule:
            rules_sr = RuleDataset(self.cgc, "new_triple_premises_sr", self.cgc.triples_sr, list(
                self.cgc.id2relation_sr.keys()),
                                   self.nega_n_r)
            rules_tg = RuleDataset(self.cgc, "new_triple_premises_tg", self.cgc.triples_tg, list(
                self.cgc.id2relation_tg.keys()),
                                   self.nega_n_r)
            self.rules_data_sr = rules_sr.get_all()
            self.rules_data_tg = rules_tg.get_all()

            if self.args.device != 'cpu':
                self.rules_data_sr = [data.cuda() for data in self.rules_data_sr]
                self.rules_data_tg = [data.cuda() for data in self.rules_data_tg]
        with tqdm(total=self.args.epoch) as _tqdm:
            for i in range(self.args.epoch):
                ...
                if self.args.dist and not self.args.only_test: # distribute on the gpu, there we not
    ...
    def train(self, _tqdm):
        ...
        triples_sr_ = TripleDataset(self.triples, self.nega_n_r)
        for batch in self.train_dataloader:
            loss, output = self.model(batch,self.rules_data_sr,self.rules_data_tg, triples_sr_.get_all())
...
if __name__ == '__main__':
    ...
    if cfgs.device == "cuda":
        ...
        torch.set_default_device(cfgs.device)
    # cfgs.device = torch.device(cfgs.device)
