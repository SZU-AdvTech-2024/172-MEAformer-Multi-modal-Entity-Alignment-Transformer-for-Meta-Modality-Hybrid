...
class cfg():
    ...
    def get_args(self):
        ...
        # base
        """
        action 被设置为 "store_false"，
        这意味着当命令行中出现 --w_rel 参数时，
        将 w_rel 属性设置为 False。如果 --w_rel
        参数没有在命令行中出现，则 w_rel 属性将使用
        default 参数指定的值，即 True。
        action : 
            count:运行这个脚本并且传递了 --verbose --verbose 那么args.verbose = 2
            append : 
        ...
        # --------- EA -----------
        parser.add_argument("--data_choice", default="DBP15K", type=str, choices=["DBP15K", "DWY", "FBYG15K", "FBDB15K", "DBPWD"], help="Experiment path")
        ...
        # ------------ Rule ------------
        parser.add_argument('--rule', type=int, default=0, help='whether to use rule')

        # ------------ Rel_weight ------------
        parser.add_argument('--rel_weight' , type=int, default=0, help='whether to use rel_weight')
