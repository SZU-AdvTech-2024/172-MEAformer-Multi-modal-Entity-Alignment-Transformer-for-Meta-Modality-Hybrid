...
class icl_loss(nn.Module):
    ...
    def softXEnt(self, target, logits, replay=False, neg_cross_kg=False):
        ...
        if replay:
            logits = logits
            idx = torch.arange(start=0, end=logprobs.shape[0], dtype=torch.int64).to(self.device)
            stg_neg = logits.argmax(dim=1)
            new_value = torch.zeros(logprobs.shape[0]).to(self.device)
            index = (
                idx,
                stg_neg,
            )
    ...
    def forward(self, emb, train_links, neg_l=None, neg_r=None, norm=True):
        ...
        if self.replay:
            ...
            if neg_l is not None:
                ...
                #
                a_ea_cand = torch.cat([train_links[:, 1], train_links[:, 0], neg_l]).to(self.device)
                b_ea_cand = torch.cat([train_links[:, 0], train_links[:, 1], neg_r]).to(self.device)
            else:
                loss_b, b_neg_idx = self.softXEnt(labels, logits_b, replay=True, neg_cross_kg=self.neg_cross_kg)
                a_ea_cand = torch.cat([train_links[:, 1], train_links[:, 0]]).to(self.device)
                b_ea_cand = torch.cat([train_links[:, 0], train_links[:, 1]]).to(self.device)
        ...
        # 对比学习里面的减号，或者说拉开距离体现在target = 0,1 上，1 就是拉近，0就是疏远。
