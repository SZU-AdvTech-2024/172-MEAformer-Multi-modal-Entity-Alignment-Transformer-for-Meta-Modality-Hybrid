...
class MformerFusion(nn.Module):
    def __init__(self, args, modal_num, with_weight=1):
        ...
        self.type_id = torch.tensor([0, 1, 2, 3, 4, 5]).to(self.args.device)

    def forward(self, embs):
        embs = [embs[idx] for idx in range(len(embs)) if embs[idx] is not None]
        modal_num = len(embs)

        hidden_states = torch.stack(embs, dim=1) # not concat is stack in a certain dim
...
class MultiModalEncoder(nn.Module):
    ...
    def forward(self,
        ...
        if self.args.rel_weight:
            if self.args.w_gcn:
                adj_r = self.sp_twin_adj(rel_emb[:19993], rel_emb[19993:])
                gph_emb = self.cross_graph_model(self.entity_emb(input_idx), adj_r)
            else:
                gph_emb = None
        else:
