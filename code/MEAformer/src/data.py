...
def load_data(logger, args):
    assert args.data_choice in ["DWY", "DBP15K", "FBYG15K", "FBDB15K", "DBPWD"]
    if args.data_choice in ["DWY", "DBP15K", "FBYG15K", "FBDB15K","DBPWD"]:
        KGs, non_train, train_ill, test_ill, eval_ill, test_ill_, ent2id_dict, triples = load_eva_data(logger, args)

    elif args.data_choice in ["FBYG15K_attr", "FBDB15K_attr"]:
        pass

    return KGs, non_train, train_ill, test_ill, eval_ill, test_ill_,ent2id_dict, triples
...
def load_eva_data(logger, args):
    ...
    # TODO : =====================================================================================================#
    # adj = args.sp_twin_adj(rel_embedding_sr, rel_embedding_tg)
    # pdb.set_trace()
    ...
    }, {"left": left_non_train, "right": right_non_train}, train_ill, test_ill, eval_ill, test_ill_, ent2id_dict, triples
...
def load_word_char_features(node_size, word2vec_path, args, logger):
    """
    node_size : ent num word2vec_path : glove.6B.300d.txt
    """
    ...
    for i, name in ent_names:
        ...
        if k:
            ...
            #  生成一个包含300个随机浮点数的数组，这些数字均匀分布在区间[-0.5, 0.5)中
...
def read_raw_data(file_dir, lang=[1, 2]):
    ...
    def get_tri(file_paths):
        tups = []
        rel2id = {}
        id2rel = {}
        p = file_dir + "/relation2id.txt"
        ...
        for file_path in file_paths:
            with open(file_path, "r", encoding="utf-8") as fr:
                for line in fr:
                    ...
                    r = line.strip('\n').split('\t')[1]
                    t = line.strip('\n').split('\t')[2]
                    if h in ent2id_dict and t in ent2id_dict and r in rel2id:
                        tups.append((ent2id_dict[h], rel2id[r], ent2id_dict[t]))
        return tups
    use = False
    if "zh_en" in file_dir and use:
        triples_ = get_tri([file_dir + "/out_zh", file_dir + "/out_en"])
        triples += triples_
        # 10576806
        # 13838806
    r_hs, r_ts = {}, {}
...
load_char_bigram(r"D:\AAA\code\MEAformer\data\mmkg\DBP15K\translated_ent_name\dbp_fr_en.json")
